...
class RNN():
    ...
    def __init__(self, n_species, n_metabolites, n_controls, n_hidden, N,
        ...
        def Ainv_next(G, Ainv, BetaInv):
            ####Does A inverse cause the long computation time??????
            GAinv = G@Ainv
    ...
    # fit to data
    def fit(self, data, trial = None,lr=1e-2, map_tol=1e-3, evd_tol=1e-3, patience=3, max_fails=3):
        ...
        while not evidence_converged:
            ...
            while not converged:
                ...
                if epoch%10==0:
                    ...
                    print(loss)
                if np.isnan(loss) and trial is not None:
                    raise Exception('loss is nan at trial '+str(trial))
    ...
    def predict(self, data, batch_size=512, t_eval = []):
    ...
    def predict_point(self, data):
        # make point predictions #This is the logic step that restricts negative numbers
    ...
    def fast_search(self, data, objective, scaler, N,
                    P=None, batch_size=512, explore = 1e-4, max_explore = 1e4):
        '''
        #DID a experiment and found batch_size 550 to be fastest but putting it back to 512
        data: main data set -> 3 dimensions (n_samples, n_time, n_outputs + n_controls)
        objective: function to maximize
        scaler: scaler object to provide processing methods
        N: ????
        P: List of objective parameters
        batch_size: used to process data in batches
        explore: scalar provided to the exploration weights
        max explore: scalar provided to the exploration weights won't go above this
        '''
        start = time.time()
        # determine number of samples to search over
        n_samples = data.shape[0]
        ...
        # compute profit function (f: R^[n_t, n_o] -> R) in batches
        #Intializing and optimizing the objective/profit function
        if P is not None:
            objective_batch = jit(vmap(lambda pred, obj_params: objective(scaler.inverse_transform(pred), obj_params)))
        else:
            objective_batch = jit(vmap(lambda pred, obj_params: objective(scaler.inverse_transform(pred))))
        ...
        f_P = []
        #Computes profit predicitons of the avalaible points in the next design
        for batch_inds in np.array_split(np.arange(n_samples), n_samples//batch_size):
            # make predictions on data
            preds = self.predict_point(data[batch_inds])
            f_P.append(objective_batch(preds, P[batch_inds]))
        ...
        best_experiments = [np.argmax(f_P).item()]
        print(f"Picked experiment {len(best_experiments)} out of {N}")
        print('Since Start ' + str(time.time() - start))
        ...
        print('Begin Loop: ' + str(time.time() - start))
        # search for new experiments until find N
        while len(best_experiments) < N:
            ...
            for batch_inds in np.array_split(np.arange(n_samples), n_samples//batch_size):
                ...
                ####This develops the EIG before the sum and the determinant
            ...
            print('EIG: ' + str(time.time() - EIG_time))
            f_I = jnp.concatenate(f_I).ravel()
            ...
            iterat = 0
            w_time = time.time()
            #Acquistion function, This loop continues until maximum predicted is not longer the maximum with the addition of the experected infomration gain or until w > max_explore
            while jnp.argmax(f_P + w*f_I) in best_experiments and w < max_explore:
                '''
                if w == 0:
                    w += explore
                ...
                w += explore
                w *= 2
            print('Weight: ' + str(time.time()-w_time))
            utilities = f_P + w*f_I
            print("Exploration weight set to: {:.4f}".format(w))
            print("Top 5 utilities: ", jnp.sort(utilities)[::-1][:5])
            ...
            for exp in exp_sorted:
                ...
                if exp not in best_experiments:
                    ...
                    for Gt in Gi:
                        Ainv_q -= self.Ainv_next(Gt, Ainv_q, self.BetaInv)
                    print(f"Picked experiment {len(best_experiments)} out of {N}")
                    print('Select: ' + str(time.time()-select))
...
class GP:
    ...
    def fit(self, X, y, tol=1e-12): #TODO was tol=1e-5
    ...
    def predict(self, Xtest):
        # Predict mean and standard deviation from kernel
    ...
    def conditioned_stdv(self, Xtest, X, Cinv):
        # Leaves out the hyperparameter self.beta
        # k_ij = kernel(X_test[i], X_train[j])
