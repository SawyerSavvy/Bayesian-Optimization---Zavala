...
class BO():
    ...
    def optimizergp(self, trials, scaling_factor, cores = 4, xinit = None):
        ...
        #xnxts should be the x-value solution to the optimization of the LCB. Why is it an array?
        xnxts = np.array([res.x for res in opt], dtype  = 'float')

        #y-values of the LCBgp (uncertainity levels)
        ...
        for i in range(self.trialsgp):
            ...
            #evluating the time it takes to computes y-value
            endf = time.time()
            ...
            #ranomly collects a distribution of numbers between 0 & sf in the matrix of 128 by self.dim
            x0 = np.random.uniform(0, sf, (128, self.dim))
            # using multiple parallel cpus for the function minimize which is analyzing the LCBgp function
            opt = Parallel(n_jobs = cores)(delayed(minimize)(LCBgp, x0 = start_point,
                                                      method = 'L-BFGS-B',
            ...
            #results of the optimization for y values
            funs = np.array([np.atleast_1d(res.fun)[0] for res in opt])
   ...
   # ####     
    def optimizersplt(self, trials, partition_num, partition_cons, scaling_factor, fcores = 4, afcores = 4, xinit = None):
        ...
        #### I think this is the contraint function. It it is outside the constraint, than a large number is returned
        def intpts(x, i):
            l = str(i+1)
        ...
        for i in range(self.split):
            ...
            if xinit is not None and switch == True:
                ...
                if n == len(self.cons[str(i+1)]):
                    ...
                    switch = False
            # if xinit is none and n dosen't equal the length, randomly select the nubmer of intial xs than select the minimum to hold as the minimum x-value
            if n!=len(self.cons[str(i+1)]):
                x0 = np.random.uniform(0, sf, (10, self.dim))

                #minimizing the acquisiton function values
            ...
            #adding the new x-value
            x = np.vstack([x, x0.reshape(1, self.dim)])
        #Number of grids per core
        splt = int(x.shape[0]/fcores)
        #best x-value per core array
        xbs = np.array(np.ones(fcores), dtype = tuple)

        #assiging xbs a x-value for each grid
        if fcores == 1:
            xbs[0] = x
        else:
        ...
        #Finds number of intial points for parallel to start at
        init_pts = max(1, int(round(128/self.split, 0)))
        #Generates the number of initial points
        ...
        for i in range(self.split):
            ...
            #Did the optimizer exit sucessfully
        ...
        #creates the next best x-values array
        ...
        for i in range(self.trialsplt):
            ...
            #finds the next best x-values for each grid
            if fcores == 1:
                xnxtbs[0] = xnxt
            else:
            ...
            # Adds the smallest y-value to the best y-values array
            ...
            #Generate random starting points for the converger method
            x0 = np.random.uniform(0, sf, (init_pts, self.dim))

            for j in range(self.split):
                #Miniminzing the acquisiton function
                #Minimize the acqusition function but stay within the bounds
                opt = Parallel(n_jobs = afcores)(delayed(minimize)(LCB, x0 = start_points,
                                                              method = 'SLSQP',
                                                              bounds = self.bounds,
                ...
                #optimized x-value
                #Find the best x-values when starting at all the different starting points
                xnxts = np.array([res.x for res in opt], dtype = 'float')

                #optimized y-value from the acqusition function for each starting point
                funs = np.array([np.atleast_1d(res.fun)[0] for res in opt])

                sts = np.array([res.success for res in opt])
                
                #Add large number to places where the converger failed
                funs[np.where(sts==False)] = max(1e6, np.max(funs))
                
                #finds the most optimized value
